...
const uploadDir = path.join(currentDir, "uploads_sql");

const sqlite3 = require('sqlite3').verbose();

async function processSQLRequest(req, res) {
  ...
  bb.on("close", async () => {
    if (hasInvalidFile) {
      ...
      res.statusCode = 400;
      res.end(
        "Invalid files found. Please only submit JavaScript files to this request."
      );
      return;
    }
    ...
    try {
      ...
      const finalResponse = await processUploadedFiles(
        uploadedFiles,
        unitTestFile
      );
...
module.exports = processSQLRequest;
